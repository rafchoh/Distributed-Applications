using ApplicationService.DTOs;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.Serialization;
using System.ServiceModel;
using System.Text;

namespace WcfService
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "IService1" in both code and config file together.
    [ServiceContract]
    public interface IService1
    {
        [OperationContract]
        string GetData(int value);

        [OperationContract]
        CompositeType GetDataUsingDataContract(CompositeType composite);

        // TODO: Add your service operations here

        // Book
        [OperationContract]
        List<BookDTO> GetBooks();

        [OperationContract]
        List<BookDTO> GetBookBySearch(string search);

        [OperationContract]
        BookDTO GetBookByID(int id);

        [OperationContract]
        string PutBook(BookDTO bookDTO);

        [OperationContract]
        string PostBook(BookDTO bookDto);

        [OperationContract]
        string DeleteBook(int id);

        // User Info
        [OperationContract]
        List<UserInfoDTO> GetUserInfo();

        [OperationContract]
        UserInfoDTO GetUserByID(int id);

        [OperationContract]
        string PutUserInfo(UserInfoDTO userInfoDTO);

        [OperationContract]
        string PostUserInfo(UserInfoDTO userInfoDto);

        [OperationContract]
        string DeleteUserInfo(int id);

        // Cart
        [OperationContract]
        List<CartDTO> GetCart();

        [OperationContract]
        CartDTO GetCartByID(int id);

        [OperationContract]
        string PutCart(CartDTO cartDto);

        [OperationContract]
        string PostCart(CartDTO bookDto);

        [OperationContract]
        string DeleteCart(int id);

    }
}

// Use a data contract as illustrated in the sample below to add composite types to service operations.
// You can add XSD files into the project. After building the project, you can directly use the data types defined there, with the namespace "WcfService.ContractType".
[DataContract]
public class CompositeType
{
    bool boolValue = true;
    string stringValue = "Hello ";

    [DataMember]
    public bool BoolValue
    {
        get { return boolValue; }
        set { boolValue = value; }
    }

    [DataMember]
    public string StringValue
    {
        get { return stringValue; }
        set { stringValue = value; }
    }
}
